...
function App() {
    const [latestId, setLatestId] = useState(
        sampleOrders[sampleOrders.length - 1].id,
    )
    ...
    const [editableOrderId, setEditableOrderId] = useState<Order["id"] | null>(
        null,
    )
    ...
    const renameOrder = (id: Order["id"]) => {
        ...
        setOrders((orders) =>
            orders.map((order) =>
                order.id === id ? { ...order, order: newName } : order,
            ),
        )
    }
    ...
    const handleFormSubmit = (newOrder: Order) => {
        setOrders((orders) =>
            newOrder.id === latestId
                ? orders.concat(newOrder)
    ...
    const handleFormCancel = () => {
        setLatestId((latestId) =>
            orders.some((o) => o.id === latestId) ? latestId : latestId - 1,
        )
        setEditableOrderId(null)
    }
    ...
    return (
        <div className="App">
            ...
            {editableOrderId ? (
                <OrderEditForm
                    order={
                        orders.find((o) => o.id === editableOrderId) || {
                            id: editableOrderId,
                        }
                    }
